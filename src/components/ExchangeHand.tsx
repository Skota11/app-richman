...
export default function HandFan({ hand, roomId, gameState, exchangeNum }: HandFanProps) {
    ...
    const handlePlay = () => {
        if (selected.length !== exchangeNum) return;
        socket?.emit("exchange-cards", { cards: selected, roomId });
        setSelected([]);
        setIsExchanged(true);
    };
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {exchangeNum > 0 && !isExchanged && (
                <div className="flex justify-center gap-2">
                    <Button
                        type="button"
                        onClick={handlePlay}
                        disabled={selected.length !== exchangeNum}
